...
def get_dst_dataset(dst_img, cols, rows, layers, dtype, proj, gt):
    ...
    try:
        ...
        if dst_img.is_file():
            dst_img.unlink()
        dst_ds = driver.Create(str(dst_img), cols, rows, layers,  # GDAL requires str
...
def xarray2geotiff(xarray, data_var, out_dir, index):
    ...
    fname = out_dir / f"forest_{index}.tif"
...
def _download_tile(forest, shapely_geom, proj, scale):
    ...
    shapely_geom : shapely.geometry
        Bounding box geometry of the tile.
    proj : str
        Projection (CRS) string.
    scale : float
        Pixel scale in units of ``proj``.

    Returns
    -------
    xarray.Dataset
        The downloaded dataset cast to bytes with renamed dimensions.

    Notes
    -----
    Retries up to 3 times with exponential backoff (2s, 4s, 8s...)
    on any exception. This handles transient GEE network errors.
    """
...
def geeic2geotiff(index, extent, ntiles, forest,
    ...
    The GEE download is retried up to 3 times with exponential backoff
    in case of transient network errors, via :func:`_download_tile`.
    """
    ...
    if (not ofile.is_file()) or (ofile.stat().st_size == 0):
        ...
        # Write data to geotiff with rioxarray
        da = ds[var_name]
        gdal_args = {
        ...
        da.rio.to_raster(str(ofile), driver="GTiff",  # rioxarray requires str
