...
fig.savefig("tmf_10yr.png", bbox_inches="tight", dpi=100)
plt.close(fig)
#+end_src

#+attr_rst: :width 800 :align center
[[file:tmf_10yr.png.png]]
...
# Export
res_df.to_csv(opj("fcc_statistics_10yr.csv"), index=False)
...
|----------+---------------------------------------------+---------+---------|
|        0 | stable non-forest                           | 2703354 |  243302 |
|        1 | stable forest                               | 1387668 |  124890 |
|        2 | forest --> deforested                       |   69043 |    6214 |
|        3 | non-forest --> old regrowth                 |   22431 |    2019 |
|        4 | forest --> old regrowth (via deforestation) |       0 |       0 |
|        5 | stable old-regrowth                         |   11484 |    1034 |
|        6 | old regrowth --> deforested                 |     866 |      78 |
...
# Export
res_df.to_csv(opj("loss_gain_statistics_10yr.csv"), index=False)
...
|------------+---------+------------------+--------------------|
| forest_t1  |  132216 |              nan |                nan |
| forest_t2  |  127943 |              nan |                nan |
| gross loss |   -6292 |             -629 |              -0.47 |
| gross gain |    2019 |              202 |               0.15 |
| net loss   |   -4273 |             -427 |              -0.32 |

When considering regrowth of at least 5 years, which is quite short for forest recovery [@], the gain (458 ha/yr) compensates the forest loss (-630 ha/yr), and the net deforestation is small (-172 ha/yr). If we consider regrowth of at least 10 years as forest (~min_years=10~ in function =get_fcc_loss_gain=), the gain is much smaller (202 ha/yr) and the net deforestation much higher (-427 ha/yr corresponding to -0.32 %/yr).
